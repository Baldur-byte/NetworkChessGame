...
// Type: GameFramework.Debugger.DebuggerManager
...
namespace GameFramework.Debugger
{
  ...
  internal sealed class DebuggerManager : GameFrameworkModule, IDebuggerManager
  {
    ...
    {
      get => this.m_ActiveWindow;
      set => this.m_ActiveWindow = value;
    }

    /// <summary>调试器窗口根结点。</summary>
    public IDebuggerWindowGroup DebuggerWindowRoot => (IDebuggerWindowGroup) this.m_DebuggerWindowRoot;
    ...
    public bool UnregisterDebuggerWindow(string path) => this.m_DebuggerWindowRoot.UnregisterDebuggerWindow(path);

    /// <summary>获取调试器窗口。</summary>
    /// <param name="path">调试器窗口路径。</param>
    /// <returns>要获取的调试器窗口。</returns>
    public IDebuggerWindow GetDebuggerWindow(string path) => this.m_DebuggerWindowRoot.GetDebuggerWindow(path);

    /// <summary>选中调试器窗口。</summary>
    /// <param name="path">调试器窗口路径。</param>
    /// <returns>是否成功选中调试器窗口。</returns>
    public bool SelectDebuggerWindow(string path) => this.m_DebuggerWindowRoot.SelectDebuggerWindow(path);
    ...
    private sealed class DebuggerWindowGroup : IDebuggerWindowGroup, IDebuggerWindow
    {
      ...
      public int DebuggerWindowCount => this.m_DebuggerWindows.Count;
      ...
      public IDebuggerWindow SelectedWindow => this.m_SelectedIndex >= this.m_DebuggerWindows.Count ? (IDebuggerWindow) null : this.m_DebuggerWindows[this.m_SelectedIndex].Value;
      ...
      public void OnEnter() => this.SelectedWindow.OnEnter();

      /// <summary>离开调试器窗口。</summary>
      public void OnLeave() => this.SelectedWindow.OnLeave();

      /// <summary>调试组轮询。</summary>
      /// <param name="elapseSeconds">逻辑流逝时间，以秒为单位。</param>
      /// <param name="realElapseSeconds">真实流逝时间，以秒为单位。</param>
      public void OnUpdate(float elapseSeconds, float realElapseSeconds) => this.SelectedWindow.OnUpdate(elapseSeconds, realElapseSeconds);
      ...
      public string[] GetDebuggerWindowNames() => this.m_DebuggerWindowNames;
      ...
      public IDebuggerWindow GetDebuggerWindow(string path)
      {
        ...
        return ((DebuggerManager.DebuggerWindowGroup) this.InternalGetDebuggerWindow(name))?.GetDebuggerWindow(path1);
      }
      ...
      {
        int length = !string.IsNullOrEmpty(path) ? path.IndexOf('/') : throw new GameFrameworkException("Path is invalid.");
        if (length < 0 || length >= path.Length - 1)
        {
